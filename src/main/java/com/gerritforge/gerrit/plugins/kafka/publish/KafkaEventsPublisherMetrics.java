...
public class KafkaEventsPublisherMetrics extends KafkaEventsMetrics {
  ...
  private static final String REQUEUE_SUCCESS_COUNTER = "broker_msg_requeue_success_counter";
  private static final String REQUEUE_FAILURE_COUNTER = "broker_msg_requeue_failure_counter";
  ...
  public KafkaEventsPublisherMetrics(MetricMaker metricMaker) {
    ...
    this.brokerRequeueSuccessCounter =
        metricMaker.newCounter(
            ...
            new Description("Number of messages that the broker publisher requeued successfully")
                .setRate()
                .setUnit("messages"),
            stringField(REQUEUE_SUCCESS_COUNTER, "Broker message requeue count"));
    this.brokerRequeueFailureCounter =
        metricMaker.newCounter(
            "kafka/broker/broker_message_requeue_failure_counter",
            new Description("Number of messages that the broker publisher failed to requeue")
                .setRate()
                .setUnit("errors"),
            stringField(REQUEUE_FAILURE_COUNTER, "Broker failed to requeue message count"));
  }
  ...
  public void incrementBrokerRequeuedMessage() {
    brokerRequeueSuccessCounter.increment(REQUEUE_SUCCESS_COUNTER);
  }

  public void incrementBrokerFailedToRequeueMessage() {
    brokerRequeueFailureCounter.increment(REQUEUE_FAILURE_COUNTER);
  }
}
