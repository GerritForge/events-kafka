      ...
      try {
        while (!closed.get()) {
          ...
          if (commitCoordinator != null) {
            // The coordinator decides when a commit is due, but Kafka commitSync must run from
            // this consumer thread because KafkaConsumer is not thread-safe.
            commitCoordinator.commitIfDue();
          }
        }
